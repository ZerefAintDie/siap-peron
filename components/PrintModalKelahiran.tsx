...
export default function PrintModalKelahiran({ data, onClose }: { data: any; onClose: () => void }) {
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/60 backdrop-blur-sm overflow-y-auto py-6 px-4">
      ...
      <div className="no-print fixed top-4 right-4 flex gap-2 z-50">
        <button
          onClick={handlePrint}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-4 py-2.5 rounded-xl shadow-lg transition-all"
        >
          Cetak / Print
        </button>
        <button
